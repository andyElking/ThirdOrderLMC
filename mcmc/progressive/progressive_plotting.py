...
def plot_progressive_results(
    ...
    if (w2 is not None) and plot_w2:
        ...
        axs[i].set_yscale("log")
        axs[i].set_ylabel("2-Wasserstein error")
...
if __name__ == "__main__":
    ...
    for name in names:
        ...
        save_name = f"../progressive_results/paper_plots/{name}_final.pdf"
        # save_name = (f"progressive_results/good_plots/"
        #              f"paper_versions/{name}_paper_version.pdf")
        figs = make_figs(latest_dict, save_name, False, True, ("QUICSORT_ADAP",))
